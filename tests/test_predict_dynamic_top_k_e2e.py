"""
End-to-end test for `Client.predict_dynamic_top_k` against a live API.

Hits the full path:

    SDK → Cloudflare → API (Cloud Run) → /v1/predict_dynamic_top_k
        → embed pod (BGE-large embedder + AlphaKeepMLP triple-head)
        → response
...
# /v1/predict_dynamic_top_k is one warm GPU forward over an L4. p50
# should be well under 100ms; we budget 1s to absorb a Cloud Run cold
# start on the first call.
_BUDGET_S = 1.0


pytestmark = pytest.mark.skipif(
    not os.environ.get(_API_KEY_ENV),
    reason=f"set {_API_KEY_ENV} to run predict_dynamic_top_k E2E",
)
...
def test_predict_dynamic_top_k_returns_valid_result():
    """Happy path: text-only call returns a DynamicTopKResult with both
    Ks in [1, 10] and alpha in [0, 1]. Latency under budget."""
    client = _client()

    # Warmup — absorbs Cloud Run cold start + BGE encoder warmup.
    _ = client.predict_dynamic_top_k("warmup probe — anything")

    t0 = time.perf_counter()
    r = client.predict_dynamic_top_k("who founded apple computer?")
    ...
    assert elapsed < _BUDGET_S, (
        f"warm predict_dynamic_top_k {elapsed:.2f}s >= {_BUDGET_S:.2f}s"
    )
    print(f"\n[e2e] predict_dynamic_top_k: {elapsed*1000:.0f} ms "
          f"alpha={r.alpha:.3f} K_dense={r.top_k_dense} K_hybrid={r.top_k_hybrid}")


def test_predict_dynamic_top_k_respects_query_vector():
